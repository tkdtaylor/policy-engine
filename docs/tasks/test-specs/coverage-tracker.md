...
| 002 | Dynamic risk scoring behind the AuthZEN seam | `002-dynamic-risk-scoring-test-spec.md` | TC-001…TC-011 | 🟡 | L5: `go build ./... && go test ./...` → `ok github.com/tkdtaylor/policy-engine`; OPA present — all 11 risk-scoring TCs ran for real (no skips). L6-via-binary: `decide --evaluator opa` with risk=0.1→`tier_select=bubblewrap`, risk=0.5→`gvisor`, risk=0.9→`firecracker`; `memory_flags=["injection-suspected"]`→`vault_injection_floor=proxy`. Verified by: L4: CI N/A (local-only repo); L5+L6 executor-observed. |
