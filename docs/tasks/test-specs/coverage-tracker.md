...
| 005 | Wire evaluator selection into the binary (`--evaluator allowlist\|opa`) | `005-evaluator-selection-binary-test-spec.md` | TC-001…TC-009 | 🟡 | L5: `go build ./... && go test -count=1 ./...` → `ok github.com/tkdtaylor/policy-engine`; TC-001…TC-009 pass (OPA-backed cases ran for real, not skipped — incl. `TestServeOPASocketRoundTrip` allow/deny over the Unix socket). L6-via-binary: `decide --evaluator opa --host api.example.com` → allow + 3 obligations, exit 0; `--host evil.example.net` → deny, exit 1; default/`--evaluator allowlist` byte-identical; `serve --evaluator opa` socket round-trip returned OPA-backed allow/deny + ping; unknown `--evaluator` and `serve` init failure → exit 1, socket unbound (no allowlist fallback). Awaiting spec-verifier APPROVE before ✅. |
