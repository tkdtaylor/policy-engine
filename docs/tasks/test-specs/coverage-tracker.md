...
|---------|---------|-----------|---------------|--------|-------------|
| 001 | Adopt OPA (Rego) behind the AuthZEN decide() seam | `001-opa-rego-evaluator-test-spec.md` | TC-001…TC-007 | 🟡 | L5: `go test ./...` → `ok github.com/tkdtaylor/policy-engine`, OPA integration test ran for real (not skipped); L6: OPA-backed decide printed allow (`vault_injection_floor=proxy`, `tier_select=bubblewrap`, `audit_emit=true`) + deny (empty obligations) |
